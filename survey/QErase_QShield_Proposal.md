...
$$
\boxed{\text{first attack (and matching certified-style defense) that treats the } \textit{deployment-realizable quantization error set}\text{ itself as the adversary's action space for diffusion ownership verification}}
$$
...
1. Attacker embeds K independent SURROGATE instances of the target carrier type
   ...
   (e.g., K different Stable-Signature decoder fine-tunes, or K different
   AquaLoRA keys) — attacker has full white-box access to these surrogates only.
...
- **Model Ownership Verification (MOV) targets** (primary): AquaLoRA-style parameter watermark, FingerInv-style intrinsic fingerprint, **Cert-LAS as the strongest MOV baseline/opponent** — QErase's headline result is beating Cert-LAS's robustness claim specifically, not a weaker baseline.
...
$$
\text{Fixed PTQ} \;<\; \text{Random adaptive PTQ} \;<\; \text{Sensitivity-search PTQ (AQ-WM-style)} \;<\; \boxed{\text{QErase}}
$$
This is the ladder that answers the reviewer's most likely objection ("just a hyperparameter search that happens to hurt watermarks") — QErase must beat sensitivity-guided search at the *same* budget, not merely beat a fixed baseline.

### 6.4 Three pilot results required before committing to full-scale experiments
1. **QErase beats sensitivity-search PTQ** at equal budget (if not, steering adds nothing over ranking+search).
...
| C3 — Secret-agnostic transfer | Precise held-out-secret transfer protocol (Section 3.4): frozen attack, same-/cross-implementation breakdown, reported under a fixed utility/stealth budget |
